...
test('component metadata matches the public surface', () => {
  ...
  assert.equal((chacha20Poly1305 as unknown as Record<string, unknown>).createEncryptor, undefined);
  assert.equal((chacha20Poly1305 as unknown as Record<string, unknown>).createDecryptor, undefined);
  ...
  assert.equal((xchacha20Poly1305 as unknown as Record<string, unknown>).createEncryptor, undefined);
  assert.equal((xchacha20Poly1305 as unknown as Record<string, unknown>).createDecryptor, undefined);
