...
import type { DeriveOptions } from '@jscrypto/core';
...
function vectorSalt(salt: string | undefined): Uint8Array | undefined {
  if (salt === undefined) {
    return undefined;
  }
...
function vectorInfo(info: string): Uint8Array {
...
test('validation rejects missing or invalid params', () => {
  ...
  assert.throws(
    () => deriveHkdf({ hash: sha256, length: 16 } as unknown as Parameters<typeof deriveHkdf>[0]),
    /HKDF requires input\./,
  );
  assert.throws(
    () => deriveHkdf({ input: null, hash: sha256, length: 16 } as unknown as Parameters<typeof deriveHkdf>[0]),
    /HKDF requires input\./,
  );
  assert.throws(
    () => deriveHkdf({ input: hex('0b'), hash: null, length: 16 } as unknown as Parameters<typeof deriveHkdf>[0]),
    /HKDF requires hash\./,
  );
  assert.throws(
    () => deriveHkdf({ input: hex('0b'), hash: undefined, length: 16 } as unknown as Parameters<typeof deriveHkdf>[0]),
    /HKDF requires hash\./,
  );
  assert.throws(
    () => deriveHkdf({ input: 123 as unknown as Uint8Array, hash: sha256, length: 16 }),
    /HKDF input must be a Uint8Array or string\./,
  );
  assert.throws(
    () => deriveHkdf({ input: hex('0b'), salt: 1 as unknown as Uint8Array, hash: sha256, length: 16 }),
    /HKDF salt must be a Uint8Array or string\./,
  );
  assert.throws(
    () => deriveHkdf({ input: hex('0b'), info: {} as unknown as Uint8Array, hash: sha256, length: 16 }),
  ...
  assert.throws(
    () => expandHkdf({ hash: sha256, length: 16 } as unknown as Parameters<typeof expandHkdf>[0]),
  ...
  assert.throws(
    () => expandHkdf({
      input: hex('077709362c2e32df0ddc3f0dc47bba6390b6c73bb50f9c3122ec844ad7c2b3e5'),
      info: 1 as unknown as Uint8Array,
      hash: sha256,
      length: 16,
    }),
    /HKDF info must be a Uint8Array or string\./,
  ...
  assert.throws(
    () => extractHkdf({ hash: sha256 } as unknown as Parameters<typeof extractHkdf>[0]),
    /HKDF requires input\./,
  );
  assert.throws(
    () => extractHkdf({ input: hex('0b'), hash: null } as unknown as Parameters<typeof extractHkdf>[0]),
    /HKDF requires hash\./,
  );
