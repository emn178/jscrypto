...
import { createAesCipher, aes } from '@jscrypto/ciphers';
...
test('AES-GCM rejects invalid options and authentication failures', () => {
  ...
  assert.throws(() => registry.createEncryptor({ cipher: 'AES', mode: 'GCM', key, iv, aad: [] as unknown as Uint8Array }), /Uint8Array/);
  assert.throws(() => registry.createDecryptor({ cipher: 'AES', mode: 'GCM', key, iv, tag: [] as unknown as Uint8Array }), /Uint8Array/);
  assert.throws(() => registry.createEncryptor({ cipher: 'AES', mode: 'GCM', key, iv, tagLength: 3 }), /tagLength/);
  assert.throws(() => registry.createEncryptor({ cipher: 'AES', mode: 'GCM', key, iv, tagLength: 3.5 }), /tagLength/);
...
test('AES-GCM decryptor rejects finalize with no buffered ciphertext', () => {
  const key = hexToBytes('000102030405060708090a0b0c0d0e0f');
  const iv = hexToBytes('101112131415161718191a1b');
  const decryptor = gcm.createDecryptor({ cipher: createAesCipher(key), iv });

  assert.throws(() => decryptor.finalize(), /authentication tag/);
});

test('AES-GCM decryptor reuses a single pending chunk without concatenation', () => {
  const key = hexToBytes('000102030405060708090a0b0c0d0e0f');
  const iv = hexToBytes('101112131415161718191a1b');
  const plaintext = textToBytes('single pending chunk');
  const ciphertext = createAesGcmRegistry().encrypt({ cipher: 'AES', mode: 'GCM', key, iv, plaintext });
  const decryptor = gcm.createDecryptor({ cipher: createAesCipher(key), iv });

  assert.deepEqual(decryptor.process(ciphertext), new Uint8Array());
  assert.equal(bytesToText(decryptor.finalize()), 'single pending chunk');
});

test('AES-GCM decryptor concatenates multiple pending chunks', () => {
  const registry = createAesGcmRegistry();
  const key = hexToBytes('000102030405060708090a0b0c0d0e0f');
  const iv = hexToBytes('101112131415161718191a1b');
  const plaintext = textToBytes('multi chunk finalize');
  const ciphertext = registry.encrypt({ cipher: 'AES', mode: 'GCM', key, iv, plaintext });
  const decryptor = gcm.createDecryptor({ cipher: createAesCipher(key), iv });

  assert.deepEqual(decryptor.process(ciphertext.subarray(0, 4)), new Uint8Array());
  assert.deepEqual(decryptor.process(ciphertext.subarray(4, 12)), new Uint8Array());
  assert.equal(bytesToText(decryptor.finalize(ciphertext.subarray(12))), 'multi chunk finalize');
});

test('GCM component metadata matches the public surface', () => {
  assert.equal(gcm.kind, 'mode');
  assert.equal(gcm.name, 'GCM');
  assert.equal(gcm.requiresPadding, false);
  assert.equal(gcm.getIvSize?.(aes as BlockCipherComponent), 0);
  assert.equal(typeof gcm.createEncryptor, 'function');
  assert.equal(typeof gcm.createDecryptor, 'function');
});
